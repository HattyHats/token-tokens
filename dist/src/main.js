/* ══════════════════ SPLASH — 60FPS CYBERNETIC MESH & AUTO TRANSITION ══════════════════ */
let _splashDone = false;
let _initAppCalled = false;

function updateSplashProgress(pct, statusText) {
  const fill = document.getElementById('splFill');
  const pctEl = document.getElementById('splPct');
...
function enterApp() {
  ...
  _splashDone = true;
  updateSplashProgress(100, 'Terminal feeds online. Ready.');
  
  if (!_initAppCalled) {
    _initAppCalled = true;
    try { initApp(); } catch(e) { console.warn('initApp notice:', e); }
  }
  ...
  if (splash) {
    ...
    splash.style.transition = 'opacity .65s cubic-bezier(.16,1,.3,1), transform .65s cubic-bezier(.16,1,.3,1), filter .65s ease';
...
(() => {
  ...
  // Parallel load: kick off data fetching immediately in background
  setTimeout(() => {
    if (!_initAppCalled) {
      _initAppCalled = true;
      try { initApp(); } catch(e) { console.warn('initApp notice:', e); }
  ...
  // Smooth realistic progress sequence: ~1.6 seconds of cybernetic visuals
  let progress = 18;
  updateSplashProgress(progress, 'Connecting to real-time market data…');
  const progTimer = setInterval(() => {
    if (_splashDone) { clearInterval(progTimer); return; }
    if (progress < 92) {
      progress += Math.random() * 14 + 8;
      let text = 'Connecting to real-time market data…';
      if (progress > 30) text = 'Fetching live prices & volume…';
      if (progress > 60) text = 'Synchronizing technical charts…';
      if (progress > 85) text = 'Terminal feeds online. Ready.';
      updateSplashProgress(Math.min(96, progress), text);
    }
  }, 160);
  ...
  // At ~1.9s, automatically dissolve splash screen and reveal terminal
  setTimeout(() => {
    enterApp();
  }, 1900);

  // Backup safety net (reveals terminal under any circumstance)
  setTimeout(() => {
    if (!_splashDone) enterApp();
  }, 2800);

  // Optional: Click anywhere on splash screen to enter immediately
  c.parentElement?.addEventListener('click', enterApp);
  window.addEventListener('keydown', (e) => {
    if (!_splashDone && (e.key === 'Enter' || e.key === ' ')) enterApp();
  });
})();
...
// Kraken pair mapping for keyless, 100% CORS multi-ticker queries
const KRAKEN_PAIRS = {
  ...
  try{
    ...
    if(r.ok){
      ...
      if(key && res[key]){
        ...
        return {
          price: p || 0,
          change: o > 0 ? ((p - o) / o) * 100 : 0,
          high24: parseFloat(d.h[1]) || 0,
          low24: parseFloat(d.l[1]) || 0,
          vol: (parseFloat(d.v[1]) || 0) * (p || 1),
...
const CC_SYM = {
  ...
  'shiba-inu':'SHIB','pepe':'PEPE','floki':'FLOKI','bonk':'BONK','wif':'WIF',
  ...
  'minswap':'MIN','sundaeswap':'SUNDAE','wingriders':'WRT',
  'liqwid-finance':'LQ','indigo-dao':'INDY','world-mobile-token':'WMT',
  'iagon':'IAG','snek':'SNEK','nmkr':'NMKR','genius-yield':'GENS',
  'empowa':'EMP','lenfi':'LENFI','hosky-token':'HOSKY','optim-finance':'OPTIM'
};

// Coinbase candles granularity mapping in seconds
const CB_GRANULARITY = {
  '1': 300,       // 5m candles (24h)
  '7': 3600,      // 1h candles (7d)
  '30': 21600,    // 6h candles (30d)
  '90': 86400,    // 1d candles (90d)
  '365': 86400,   // 1d candles (350d)
  '1825': 86400,  // 1d candles
  'max': 86400    // 1d candles
};
...
async function loadChart(id, days) {
  const loadEl = document.getElementById('cLoad');
  ...
  try {
    ...
    try {
      const gran = CB_GRANULARITY[days] || 86400;
      const cbUrl = `https://api.exchange.coinbase.com/products/${sym}-USD/candles?granularity=${gran}`;
      const res = await fetch(cbUrl);
      if(res.ok){
        const raw = await res.json();
        if(Array.isArray(raw) && raw.length > 0){
          // Coinbase returns [ time(sec), low, high, open, close, volume ] in descending order
          candles = raw.map(c => ({
            t: c[0] * 1000,
            l: parseFloat(c[1]),
            h: parseFloat(c[2]),
      ...
      console.warn('Coinbase candle fetch notice:', cbErr);
    }

    // 2. Secondary fallback: Kraken OHLC endpoint
    if(!candles || !candles.length){
      try {
        const kPair = KRAKEN_PAIRS[sym] || (sym + 'USD');
        const interval = KRAKEN_INTERVAL[days] || 1440;
        const kUrl = `https://api.kraken.com/0/public/OHLC?pair=${kPair}&interval=${interval}`;
        ...
        if(res.ok){
          ...
          if(Array.isArray(raw) && raw.length > 0){
            // Kraken returns [ time(sec), open, high, low, close, vwap, volume, count ]
            candles = raw.map(c => ({
              t: c[0] * 1000,
              o: parseFloat(c[1]),
              h: parseFloat(c[2]),
        ...
        console.warn('Kraken candle fetch notice:', kErr);
      }
    }
    ...
    // Load mini charts in background without blocking
    loadMiniCharts(id, sym);
    ...
    // Show appropriate zoom window; user can drag/scroll back
    const view = Math.min(100, ST.candles.length);
    ST.zoom = { s: Math.max(0, ST.candles.length - view), r: view };
    renderChart();
  } catch(err) {
    console.warn('loadChart graceful fallback to synthCandles:', err);
    setDot('ok');
    synthCandles();
    renderChart();
  } finally {
...
function synthCandles(){
  let p = ST.coin.price || 1000;
  ST.candles = [];
  const now = Date.now();
  for(let i=90; i>=0; i--){
    const o = p;
    const v = p * 0.015;
    const cl = o + (Math.random() - 0.49) * p * 0.012 + (Math.random() - 0.5) * v;
    ST.candles.push({
      t: now - i * 86400000,
      o,
      h: Math.max(o, cl) + Math.random() * v * 0.6,
      l: Math.min(o, cl) - Math.random() * v * 0.6,
      c: cl,
      v: 1e6 + Math.random() * 9e6
    });
    p = cl;
  }
  ST.zoom = { s: 10, r: 80 };
...
const POPULAR_TOKENS = [
  ...
  { id: 'snek', name: 'Snek', symbol: 'SNEK' },
  { id: 'minswap', name: 'Minswap', symbol: 'MIN' },
  { id: 'sundaeswap', name: 'SundaeSwap', symbol: 'SUNDAE' },
  { id: 'wingriders', name: 'WingRiders', symbol: 'WRT' },
  { id: 'liqwid-finance', name: 'Liqwid', symbol: 'LQ' },
  { id: 'indigo-dao', name: 'Indigo', symbol: 'INDY' },
  { id: 'world-mobile-token', name: 'World Mobile', symbol: 'WMT' },
...
function setupSearch(){
  ...
  inp.addEventListener('input', () => {
    ...
    dd.innerHTML = '<div class="sd-msg"><div class="spin" style="display:inline-block;width:14px;height:14px;border-width:1.5px;vertical-align:middle;margin-right:6px;"></div>Searching tokens…</div>';
    dd.classList.add('show');
    ST.sbt = setTimeout(() => doSearch(q), 300);
  });
  if(xBtn) xBtn.addEventListener('click', clearSearch);
  document.addEventListener('click', e => {
    if(!e.target.closest('.sb-top')) clearSearch();
  });
}
...
async function doSearch(q){
  ...
  try {
    // 1. Instant local matching against popular tokens + watchlist
    const localMatches = POPULAR_TOKENS.concat(ST.wl || []).filter((c, idx, arr) => {
      const sym = (c.symbol || '').toLowerCase();
      const nm = (c.name || '').toLowerCase();
      return (sym.includes(qlo) || nm.includes(qlo)) &&
    ...
    // Sort: exact symbol match first, then startsWith, then general match
    const sorted = combined.sort((a, b) => {
      const as = a.symbol.toLowerCase() === qlo ? -2 : a.symbol.toLowerCase().startsWith(qlo) ? -1 : 0;
      const bs = b.symbol.toLowerCase() === qlo ? -2 : b.symbol.toLowerCase().startsWith(qlo) ? -1 : 0;
      return as - bs;
    }).slice(0, 10);

    if (!sorted.length) {
      dd.innerHTML = `<div class="sd-msg">No results for "${esc(q)}"</div>`;
      return;
    }

    // Fetch prices for sorted tokens
    const priceMap = {};
    (ST.wl || []).forEach(w => {
    ...
    dd.innerHTML = `<div class="sd-hdr"><span>${sorted.length} results</span><span>Click to view chart</span></div>` +
      sorted.map(c => {
        const sym = c.symbol;
        const d = priceMap[c.id] || priceMap[sym] || {};
        const inW = inWL.has(sym);
        const col = rc(sym);
        const img = c.thumb
          ? `<img class="sd-img" src="${c.thumb}" alt="" onerror="this.outerHTML='<div class=sd-fb style=background:${col}22;color:${col};>${sym.substring(0,3)}</div>'">`
        ...
        return `<div class="sd-r">
          ...
          <button class="sd-add${inW?' done':''}"
                  onclick="sbAddWL('${coinId}','${esc(c.name||sym)}','${sym}','${c.thumb||''}',this)">
            ${inW?'✓ Added':'+ WL'}
          </button>
        </div>`;
      }).join('');
    ...
    dd.innerHTML = '<div class="sd-msg">Search unavailable — please try again</div>';
  }
}
...
function renderWL(prev){
  ...
  el.innerHTML=ST.wl.map((c,i)=>{
    ...
    const fl=prev&&prev[c.id]&&c.price>0?(c.price>prev[c.id]?' flash-up':c.price<prev[c.id]?' flash-down':''):'';
...
function updateHdr(c, oldPrice){
  ...
  if(c.change!=null){
    const b=document.getElementById('chBadge'),s=c.change>=0?'+':'';
    b.textContent=s+c.change.toFixed(2)+'%';
    b.className='ch-badge '+(c.change>=0?'pos':'neg');
  }
...
async function loadTrending(){
  ...
  if(!trEl) return;
  try{
    // Try CoinGecko open trending endpoint
    ...
    if(res.ok){
      ...
      if(coins.length > 0){
        trEl.innerHTML = coins.map((item, i) => {
          ...
          const id = c.id || sym.toLowerCase();
          return `<div class="tr-r" onclick="selectCoin('${id}','${esc(c.name||sym)}','${sym}','${img}')">
    ...
    console.warn('Trending CoinGecko notice:', e);
  }

  // Fallback to top watchlist tokens
  if(ST.wl && ST.wl.length){
    const sorted = [...ST.wl].sort((a,b) => (b.change || 0) - (a.change || 0)).slice(0, 7);
    trEl.innerHTML = sorted.map((c, i) => {
      const sym = (c.symbol || '').toUpperCase();
      const ch = c.change;
    ...
    trEl.innerHTML = '<div style="padding:10px;text-align:center;color:var(--text3);font-size:.75rem;">Unavailable</div>';
  }
}
...
async function loadTicker(){
  ...
  if(!track) return;
  try{
    const pairs = "XXBTZUSD,XETHZUSD,SOLUSD,ADAUSD,XDGUSD,XXRPZUSD,AVAXUSD,DOTUSD,LINKUSD,XLTCZUSD,UNIUSD,NEARUSD,ATOMUSD";
    const res = await fetch(`https://api.kraken.com/0/public/Ticker?pair=${pairs}`);
    if(res.ok){
      const json = await res.json();
      const result = json.result || {};
      const symbols = {
        "XXBTZUSD": "BTC", "XETHZUSD": "ETH", "SOLUSD": "SOL", "ADAUSD": "ADA",
        "XDGUSD": "DOGE", "XXRPZUSD": "XRP", "AVAXUSD": "AVAX", "DOTUSD": "DOT",
        "LINKUSD": "LINK", "XLTCZUSD": "LTC", "UNIUSD": "UNI", "NEARUSD": "NEAR", "ATOMUSD": "ATOM"
      };
      const idMap = {
      ...
      for(const [k, v] of Object.entries(result)){
        ...
        const cid = idMap[sym] || sym.toLowerCase();
        ...
        const sign = chg >= 0 ? '+' : '';
        items.push(`<div class="ti" style="cursor:pointer;" onclick="selectCoin('${cid}','${sym}','${sym}','')">
          <b>${sym}</b> ${p_str}
    ...
    console.warn('Kraken ticker notice:', e);
  }
  ...
  if(ST.wl && ST.wl.length){
    const items = ST.wl.filter(c => c.price > 0).map(c => {
      ...
      const sign = ch >= 0 ? '+' : '';
      return `<div class="ti" style="cursor:pointer;" onclick="selectCoin('${c.id}','${esc(c.name||sym)}','${sym}','${c.image||''}')">
        <b>${sym}</b> $${fP(p)}
...
function renderChart(){
  ...
  pc.width = W * dpr; pc.height = pH * dpr;
  pc.style.width = W + 'px'; pc.style.height = pH + 'px';

  vc.width = W * dpr; vc.height = subH * dpr;
  vc.style.width = W + 'px'; vc.style.height = subH + 'px';
  ...
  if(showRSI) {
    rsiC.width = W * dpr; rsiC.height = rsiH * dpr;
    rsiC.style.width = W + 'px'; rsiC.style.height = rsiH + 'px';
  }
  if(showMACD){
    macdC.width = W * dpr; macdC.height = macdH * dpr;
    macdC.style.width = W + 'px'; macdC.style.height = macdH + 'px';
  }
  ...
  if(window._hoverCursor && !DRW.tool){
    const { x, y } = window._hoverCursor;
    if(x >= PAD_L && x <= W - PAD_R && y >= PAD_T && y <= pH - PAD_B){
      ctx.save();
      ctx.setLineDash([4, 4]);
      ...
      ctx.beginPath();
      ctx.moveTo(x, PAD_T);
      ctx.lineTo(x, pH - PAD_B);
      ctx.stroke();

      // Price badge on right axis
      const hoverPrice = maxP - ((y - PAD_T) / cH) * pRange;
      ctx.setLineDash([]);
      ctx.fillStyle = '#00d4ff';
      if(ctx.roundRect) ctx.roundRect(W - PAD_R + 1, y - 8, PAD_R - 2, 16, 3);
      else ctx.rect(W - PAD_R + 1, y - 8, PAD_R - 2, 16);
      ctx.fill();
      ctx.fillStyle = '#050810';
      ctx.font = 'bold 8.5px Share Tech Mono';
      ctx.textAlign = 'center';
      ctx.fillText('$' + fP(hoverPrice), W - PAD_R + 1 + (PAD_R - 2)/2, y + 3.5);

      // Date badge on bottom axis
      const cIdx = Math.max(0, Math.min(vis.length - 1, Math.floor((x - PAD_L) / cw)));
      const cTime = vis[cIdx]?.t;
      if(cTime){
        const dt = new Date(cTime);
        const dateTag = (dt.getMonth()+1) + '/' + dt.getDate() + ' ' + String(dt.getHours()).padStart(2,'0') + ':' + String(dt.getMinutes()).padStart(2,'0');
        const badgeW = 76;
        ctx.fillStyle = 'rgba(10, 18, 32, 0.95)';
        ctx.strokeStyle = 'rgba(0, 229, 255, 0.6)';
        ctx.lineWidth = 1;
        if(ctx.roundRect) ctx.roundRect(x - badgeW/2, pH - 18, badgeW, 16, 3);
        else ctx.rect(x - badgeW/2, pH - 18, badgeW, 16);
        ctx.fill();
        ctx.stroke();
        ctx.fillStyle = '#00d4ff';
        ctx.font = '8px Share Tech Mono';
        ctx.textAlign = 'center';
        ctx.fillText(dateTag, x, pH - 6.5);
      }
      ctx.restore();
    }
  }
...
/* ══════════════════ MINI CHARTS ════════════════════════ */
async function loadMiniCharts(coinId, symbol){
  const sym = symbol || CC_SYM[coinId] || (ST.wl.find(c => c.id === coinId)?.symbol || 'BTC').toUpperCase();
  try{
    const url = `${CC}/histoday?fsym=${sym}&tsym=USD&limit=365&aggregate=1`;
    const res = await fetch(url, {headers:{'Accept':'application/json'}});
    if(res.ok){
      const json = await res.json();
      const all = json.Data?.Data || [];
      if(all.length >= 2){
        const clean = all.map(c => ({t: c.time * 1000, o: c.open, h: c.high, l: c.low, c: c.close, v: c.volumefrom})).filter(c => c.c > 0);
        if(clean.length >= 2){
          drawMiniChart('1', clean.slice(-2));
          drawMiniChart('7', clean.slice(-7));
          drawMiniChart('30', clean.slice(-30));
          drawMiniChart('365', clean);
          return;
        }
      }
    }
  }catch(e){
    console.warn('loadMiniCharts error:', e);
  }

  // Graceful fallback to currently loaded candles
  if (ST.candles && ST.candles.length >= 2) {
    drawMiniChart('1', ST.candles.slice(-Math.min(24, ST.candles.length)));
    drawMiniChart('7', ST.candles.slice(-Math.min(50, ST.candles.length)));
    drawMiniChart('30', ST.candles.slice(-Math.min(90, ST.candles.length)));
    drawMiniChart('365', ST.candles);
  }
}
...
document.getElementById('priceCanvas').addEventListener('mousemove',function(e){
  ...
  window._hoverCursor = { x: mx, y: my };
  const vis=ST.candles.slice(Math.max(0,ST.zoom.s),Math.min(ST.candles.length,ST.zoom.s+ST.zoom.r));
  if(!vis.length)return;
  const PAD_L=8, PAD_R=82;
  const cW = rect.width - PAD_L - PAD_R;
  const cw = cW / vis.length;
  const idx = Math.max(0, Math.min(vis.length-1, Math.floor((mx - PAD_L)/cw)));
  ...
  if(lx+185>rect.width) lx=mx-195;
  ly=Math.max(0,Math.min(rect.height-155,ly));
  tt.style.left=lx+'px';
  tt.style.top=ly+'px';
...
document.getElementById('priceCanvas').addEventListener('mouseleave',()=>{
  document.getElementById('cTip').style.opacity=0;
  window._hoverCursor = null;
  if(!DRW.tool) renderChart();
});
...
async function runTimeMachine() {
  ...
  // ── Strategy 1: CryptoCompare historical day price by timestamp (100% CORS) ─
  try {
    const sym = CC_SYM[ST.coin.id] || (ST.coin.symbol || 'BTC').toUpperCase();
    const toTsSec = Math.floor(targetTs / 1000);
    const url = `${CC}/histoday?fsym=${sym}&tsym=USD&toTs=${toTsSec}&limit=1`;
    const res = await fetch(url, { headers: { 'Accept': 'application/json' } });
    if (res.ok) {
      const json = await res.json();
      ...
      if (items.length) {
        ...
        if (item && item.close > 0) {
          loadEl.classList.remove('show');
          showTMSuccess(item.close, displayDate, null, item.volumefrom || item.volumeto, 'CryptoCompare');
          return;
        }
      }
    }
  } catch (e) {
    console.warn('CC TimeMachine lookup failed:', e);
  }

  // ── Strategy 2: CoinGecko history endpoint ─────────────────────────────────
  try {
...
function F_() {
  if (window.fbSignOut && window.firebaseAuth) {
    ...
    showToast('Signed Out', 'You are now browsing in guest mode.', 'info', 2500);
  }
}
...
function toggleAuthMode(e) {
  ...
  const toggleText = document.getElementById('fbToggleText');
  ...
  if (_authMode === 'signup') {
    ...
    if (link) link.textContent = 'Sign Up';
  }
}

async function handleAuthSubmit() {
  const email = document.getElementById('fbEmail')?.value.trim();
  const pass = document.getElementById('fbPass')?.value.trim();
  const errEl = document.getElementById('fbError');
  if (errEl) errEl.textContent = '';
  ...
  if (!window.firebaseAuth || !window.fbSignInWithEmailAndPassword) {
    // Local session fallback mode if Firebase SDK isn't loaded
    showToast('👤 Signed In', 'Profile active: ' + email.split('@')[0], 'success', 3000);
    document.getElementById('fbModal').style.display = 'none';
    const t = document.getElementById('authBtn');
    if (t) {
      t.innerHTML = '👤 ' + email.split('@')[0];
      t.onclick = () => {
        t.innerHTML = '👤 Log In';
        t.onclick = () => { document.getElementById('fbModal').style.display = 'flex'; };
        showToast('Signed Out', 'You are now in guest mode.', 'info', 2500);
      };
    }
    return;
  }
  ...
  try {
    if (_authMode === 'signup') {
      if (window.fbCreateUserWithEmailAndPassword) {
        await window.fbCreateUserWithEmailAndPassword(window.firebaseAuth, email, pass);
        showToast('Account Created', 'Welcome to Token-Tokens!', 'success', 3000);
      }
    } else {
      await window.fbSignInWithEmailAndPassword(window.firebaseAuth, email, pass);
      showToast('Welcome Back', 'Logged in successfully.', 'success', 3000);
    }
    document.getElementById('fbModal').style.display = 'none';
  } catch (err) {
    console.warn('Auth error:', err);
    if (errEl) errEl.textContent = err.message ? err.message.replace('Firebase: ', '') : 'Authentication failed';
  }
}

setTimeout(() => {
  if (window.fbOnAuthStateChanged && window.firebaseAuth) {
    window.fbOnAuthStateChanged(window.firebaseAuth, async e => {
      ...
      if (e) {
        ...
        try {
          ...
          if (snapshot.exists() && snapshot.val().watchlist?.length) {
        ...
        // Do NOT wipe watchlist on guest/unauthenticated mode
        if (!ST.wl || !ST.wl.length) {
          loadState();
        }
        renderWL({});
...
function syncFirebaseWL() {
  if (P_ && window.fbRef && window.fbSet && window.firebaseDb) {
